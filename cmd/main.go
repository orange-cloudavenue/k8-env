...
func main() {
	...
	flag.StringVar(&webhookServiceName, "service-name", "k8-env", "Webhook service name.")
	flag.StringVar(&envConfigFile, "env-config-file", "/etc/webhook/config/envconfig.yaml", "Environnement variable injector configuration file.")
	flag.Parse()
	...
	envConfig, err := loadConfig(envConfigFile)
	if err != nil {
